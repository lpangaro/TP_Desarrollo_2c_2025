    ...
    return (
        <nav className="navbar">
            ...
            <div className="navbar-right">
                ...
                {user ? (
                    <>
                        <div className="accordion" id="accordionNotif">
                            <div className="accordion-item">
                                ...
                                <div id="collapseNotif" className="accordion-collapse collapse" data-bs-parent="#accordionNotif">
                        ...
                        <div className="navbar-auth">
                            {user.tipo === 'VENDEDOR' && (
                                <button className="navbar-auth-btn" onClick={() => { navigate(`/productos/vendedor/${user._id}`) }}>
                                    <BsPersonCircle className="navbar-auth-icon" />
                                    <span className="navbar-auth-text">{user.nombre}</span>
                                </button>
                            )}
                            {user.tipo === 'COMPRADOR' && (
                                <button className="navbar-auth-btn" onClick={() => { navigate('/mi-perfil') }}>
                                    <BsPersonCircle className="navbar-auth-icon" />
                                    <span className="navbar-auth-text">{user.nombre}</span>
                                </button>
                            )}
                            <button onClick={() => { logoutUser(); navigate('/') }} className='bg-danger text-light p-2 rounded-2 ms-2'>Salir <FiLogOut /></button>
                        </div>
                    </>
                ) : (
                    <div className="navbar-auth">
                        <button className="navbar-auth-btn" onClick={() => { navigate('/register') }}>
                            ...
                            <span className="navbar-auth-text">Registrarse</span>
                        </button>
                        <button className="navbar-auth-btn" onClick={() => navigate('/login')}>
                            <FaRightToBracket className="navbar-auth-icon" />
                            <span className="navbar-auth-text">Iniciar Sesión</span>
                        </button>
                    </div>
                )}
            </div>
